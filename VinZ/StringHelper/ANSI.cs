...
public static partial class StringHelper
{
    ...
    private static Regex CleanAnsiRx = new Regex(@"(\x9B|\x1B\[)[0-?]*[ -\/]*[@-~]");

    public static string RemoveAnsi(this string text)
    {
        return CleanAnsiRx.Replace(text, "");
    }

    public static bool RemoveAnsiFromConsole(CancellationToken? cancel = default)
    {
        var consoleOutSave = Console.Out;
        var consoleErrSave = Console.Error;

        var outWriter = new StringWriter();
        var outBuilder = outWriter.GetStringBuilder();
        ...
        var errPop = () =>
        {
            var s = errBuilder.ToString();
            errBuilder.Clear();
            return s;
        };

        bool Print(TextWriter dest, Func<string> func, ref string lineNoCr)
        {
            var s = func();

            var lines = s.Replace("\r", "");
            int lastCr = lines.LastIndexOf('\n');
            var print = lastCr > 0;

            if (print)
            {
                var lineCr = lineNoCr + lines.Substring(0, lastCr);
                lineNoCr = lines.Substring(lastCr + 1);

                dest.WriteLine(lineCr.RemoveAnsi());
            }
        ...
        var printTask = Task.Run(async () =>
        {
            ...
            while (true)
            {
                ...
                var outPrint = Print(consoleOutSave, outPop, ref outRemainder);
                var errPrint = Print(consoleErrSave, errPop, ref errRemainder);

                if (outPrint || errPrint)
                {
                    continue;
                }

                await Task.Delay(100, cancel ?? CancellationToken.None);
            }
    ...
    {
        var colorname = colorName.ToLower();

        for (short i = 0; i < KnownColors.ArgbValues.Length; i++)
        {
            var name = KnownColors.GetName(i).ToLower();
            if (name == colorname)
            {
                var hex = KnownColors.ArgbValues[i];
                return "#" + $"{hex:x8}".Substring(2, 6);
            }
        }
    ...
    public static ANSIString Color(this ANSIString text, string nameOrHex)
    {
        if (nameOrHex.StartsWith("#"))
        {
            var hex = int.Parse(nameOrHex.Substring(1), System.Globalization.NumberStyles.HexNumber);
            return Color(text, System.Drawing.Color.FromArgb(hex));
        }
            
        return Color(text, System.Drawing.Color.FromName(nameOrHex));
    }
    ...
    public static ANSIString Background(this ANSIString text, string nameOrHex)
    {
        if (nameOrHex.StartsWith("#"))
        {
            var hex = int.Parse(nameOrHex.Substring(1), System.Globalization.NumberStyles.HexNumber);
            return Background(text, System.Drawing.Color.FromArgb(hex));
        }
            
        return Background(text, System.Drawing.Color.FromName(nameOrHex));
    }
